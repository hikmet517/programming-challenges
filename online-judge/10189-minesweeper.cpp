    ...
    while (std::cin >> row >> col)
    {
        ...
        for (int i = 0; i < row; i++)
        {
            for (int j = 0; j < col; j++)
            {
                ...
                int northeast =
                    (i - 1 >= 0 && j + 1 < col && matrix[i - 1][j + 1] == '*') ? 1 : 0;
                int northwest =
                    (i - 1 >= 0 && j - 1 >= 0 && matrix[i - 1][j - 1] == '*') ? 1 : 0;
                int southeast =
                    (i + 1 < row && j + 1 < col && matrix[i + 1][j + 1] == '*') ? 1 : 0;
                int southwest =
                    (i + 1 < row && j - 1 >= 0 && matrix[i + 1][j - 1] == '*') ? 1 : 0;

                int count = north + south + east + west + northeast + northwest + southeast +
                            southwest;
                matrix[i][j] = '0' + count;
            }
        }
